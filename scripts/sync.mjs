...
const cfg = {
  ...
  graph: `https://graph.facebook.com/${env("GRAPH_VERSION", "v23.0")}`,
...
async function fetchAds(mediaIds) {
  ...
  // 광고 소재에 연결된 인스타그램 게시물 ID
  const mediaOf = new Map();
  const adIds = rows.map((r) => r.ad_id);
  for (let i = 0; i < adIds.length; i += 50) {
    const batch = adIds.slice(i, i + 50);
    const res = await graph("", { ids: batch.join(","), fields: "creative{effective_instagram_media_id}" });
    for (const id of batch) mediaOf.set(id, res[id]?.creative?.effective_instagram_media_id ?? null);
  }
